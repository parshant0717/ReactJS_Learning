import RestaurantCards, {
  promotedRestaurnatCards,
} from "../commn_components/RestaurantCard";
...
const Body = () => {
  ...
  const RestaurantCardsPromoted = promotedRestaurnatCards(RestaurantCards);
      ...
      <div className="res-compContainer">
        {filteredList.map((apiData) => (
          <Link key={apiData.info.id} to={`/restaurants/${apiData.info.id}`}>
            {apiData?.info?.promoted ? (
              <RestaurantCardsPromoted data={apiData.info} />
            ) : (
